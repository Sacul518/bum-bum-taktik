...
> **Session B umgesetzt (2026-07-18):** Wirtschaft mit zwei Konten (`server/src/economy.ts`; Einkommen: HQ +1 Credit/s Grundsold, Stadt +2 Credits/s, Mine +1 Material/s; Start 150/60; Anzeige im HUD oben rechts + `resources`-Befehl) · neutrale POIs Mine/Kaserne/Hafen/Flugplatz (einnehmbar, Mindestabstand 6; Hafen nur auf Kacheln mit Wasser-Nachbar, entfällt sonst) · Produktion gegen Ressourcen am passenden Gebäude (`produce`-Befehl, `PRODUCTION_BUILDING`/`UNIT_COST`/`PRODUCTION_TIME_MS` in shared, unicast `produceResult`, Baustatus im `BuildingSnapshot`; die Gratis-Infanterie der Fabriken ist ersetzt) · Karten-Deko als Instanced Meshes (Bäume/Felsen, 3 Draw-Calls, Caps 4000/2000, deterministischer Kachel-Hash, Freihalte-Zone um Gebäude) · Radar getrennt vom FoW (`RADAR_RANGE`: HQ 40, Flugplatz 30; anonyme, gerundete `radarContacts` als gelbe Hohlkreise auf der Minimap) · Feind-KI-Strategie-Schicht (alle 2 s: Infanterie nimmt fremde Gebäude ein, Kampfeinheiten belagern Spieler-Gebäude, Produktion vom eigenen Konto, Armee-Cap 16). Details: `ZUSAMMENFASSUNG.md` + Haken in `docs/PLAN.md`.
